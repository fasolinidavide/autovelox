...
passaggio2: lh $s4, 0($s6)                          #per non sporcare il valore di s6 quando andremo ad inserire manualmente il passaggio della macchina, utilizziamo un registro terzo
	...
	sh $s5, 0($s6)				# indicando lo scatto della fotocamera salvandola in s5 e poi in memoria in s6
 	j fine


controllo2: slt $t0, $s2, $s3			# verifico che sia compreso tra 55 e 60, come ho fatto nel controllo prima
	    beq $t0, $zero, controllo3		
     	    li $s5, 49792			# velocità compresa tra 55 km/h e 60 km/h -> 0000 0000 1000 0001 (128+2 = 130)
	    sh $s5, 0($s6)			# salvo il nono bit a 1 (1100 0010 1000 0000)
	    j fine

controllo3: li $s5, 50048			#l'unica alternativa è che la velocità sia >60 km/h (1100 0011 1000 0000)
	    sh $s5, 0($s6)
